...
class TpfAccumulator(val frequencyHz: Float, private val oneShot: Boolean = false, val callback: (Float) -> Unit) : TpfReceiver {
    ...
    constructor(period: Period, oneShot: Boolean = false, callback: (Float) -> Unit) :
            this(1f / period.toStandardSeconds().seconds.toFloat(), oneShot, callback)

    constructor(duration: Duration, oneShot: Boolean = true, callback: (Float) -> Unit) :
            this(1f / duration.standardSeconds, oneShot, callback)
